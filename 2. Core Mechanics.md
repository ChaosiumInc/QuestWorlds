...
This reflects the fact that an ability above 20 would always succeed on a D20. To reflect higher abilities, or resistances, we divide the ability by 20, using a mastery for each multiple of 20 the ability is, and treating the remainder as the target number. So an ability of 27 translates into 1 remainder 7 or 7M; an ability of 43 translates into 2 remainder 3 or 3M2.

Because each mastery represents automatic success (apart from a fumble) on a D20, we roll agains the remainder, and treat the mastery as a bump. So on an ability of 27 is 7M, which means a target number of 7 and bump the result; an ability of 43 means 3M2 or a target number of 3 and bump the result twice.
